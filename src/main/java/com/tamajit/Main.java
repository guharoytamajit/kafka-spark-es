...
//        pairRDD.foreachRDD(pRDD-> { pRDD.foreach(tuple-> System.out.println(tuple._2()));});

        JavaDStream<String> map = stream.map(record-> record.value());
        map.foreachRDD(rdd->{
       	
        	System.out.println(rdd); 
       	 JavaEsSpark.saveJsonToEs(rdd, "spark/json-trips");
        });
